...
class QueryVehicleStatusWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
        this.txIndex++;
        // 设置查询的车辆ID
        let id = `Client${this.workerIndex}_channeld${this.txIndex}`; // 设施唯一ID
        let args = {
            contractId: 'channela_ledger',
            contractVersion: 'v1',
            contractFunction: 'UnifiedQuery',
            contractArguments: ["channeld",id,"ParkingFacility","test"],
            timeout: 30,
            readOnly: true,
            targetOrganizations: ['Org7MSP'],
            targetPeers: ['peer0.org7.example.com']
        };
