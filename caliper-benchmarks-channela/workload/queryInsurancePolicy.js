...
class QueryVehicleStatusWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
        this.txIndex++;
        // 设置查询的车辆ID
        let id = `Client${this.workerIndex}_channelc${this.txIndex}`; // 车辆唯一ID
        let args = {
            contractId: 'channela_ledger', // 智能合约ID
            contractVersion: 'v1',  // 智能合约版本
            contractFunction: 'UnifiedQuery',  // 调用的链码函数
            contractArguments: ["channelc",id,"InsurancePolicy","test"],  // 查询的车辆ID
            timeout: 30,  // 超时时间
            readOnly: true,  // 设置为只读查询
            targetOrganizations: ['Org4MSP'],
            targetPeers: ['peer0.org4.example.com']
        };
